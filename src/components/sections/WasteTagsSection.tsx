    ...
    <section className="py-14 md:py-20 bg-white">
      <div className="container mx-auto px-4 md:px-8">
        ...
        <div {...fadeInHeader} className={`text-center mb-8 ${fadeInHeader.className}`}>
          <h3 className="text-xs font-bold text-cobalt-600 tracking-widest uppercase mb-2">수거 대상 품목</h3>
          ...
          <div className="flex flex-wrap gap-2 justify-center">
            {WASTE_TYPES.map((item) => (
              <Link
                ...
                className="px-4 py-1.5 text-sm font-medium border rounded-pill transition-colors bg-slate-50 text-slate-700 border-slate-200 hover:bg-cobalt-50 hover:text-cobalt-700 hover:border-cobalt-200"
              >
                {item}
              </Link>
            ))}
        ...
        <div className="mt-10 max-w-3xl mx-auto">
          <div className="flex items-center gap-2 mb-3 justify-center">
            <BookOpen size={15} className="text-cobalt-600" />
            <h3 className="text-sm font-bold text-navy-700">품목별 상세 가이드</h3>
          </div>
            ...
            {DETAIL_GUIDES.map((guide, i) => (
              <span key={guide.slug} className="flex items-center">
                <Link
                  ...
                  className="px-2 py-0.5 text-slate-600 hover:text-cobalt-600 underline-offset-4 hover:underline transition-colors"
                >
                  {guide.name}
                </Link>
        ...
        <div className="mt-8 text-center">
          ...
          <Link
            href="/waste/types"
            className="inline-flex items-center gap-1.5 text-cobalt-600 font-semibold text-sm hover:underline underline-offset-4"
          >
            수거 대상 폐기물 상세 안내
            <ArrowRight size={14} />
          </Link>
