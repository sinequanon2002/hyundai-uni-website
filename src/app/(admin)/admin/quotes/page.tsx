  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-bold text-navy-900">견적서 관리</h1>
      ...
      {quotations.length === 0 ? (
        ...
        <div className="bg-white rounded-sm shadow-ds-sm overflow-hidden">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50">
                  <th className="text-left px-4 py-3 text-xs text-slate-500 font-medium">견적번호</th>
                  ...
                  <th className="text-right px-4 py-3 text-xs text-slate-500 font-medium">합계금액</th>
                  <th className="text-center px-4 py-3 text-xs text-slate-500 font-medium">상태</th>
                  <th className="text-left px-4 py-3 text-xs text-slate-500 font-medium">유효기간</th>
                  <th className="text-left px-4 py-3 text-xs text-slate-500 font-medium">작성일</th>
                </tr>
              </thead>
                ...
                {quotations.map((q) => (
                  <tr key={q.id} className="hover:bg-slate-50 transition-colors">
                    <td className="px-4 py-3">
                      <Link href={`/admin/quotes/${q.id}`} className="font-mono text-xs text-cobalt-600 hover:text-mint-600 font-medium">
                        {q.quote_number}
                      </Link>
                    </td>
                    <td className="px-4 py-3">
                      <Link href={`/admin/quotes/${q.id}`} className="font-medium text-navy-900 hover:text-cobalt-600">
                        {q.company_name}
                      </Link>
                    </td>
                    <td className="px-4 py-3 text-slate-600">{q.contact_name}</td>
                    <td className="px-4 py-3 text-right font-medium text-navy-900">{fmt(q.total)}</td>
          ...
          <div className="sm:hidden divide-y divide-slate-100">
            {quotations.map((q) => (
              <Link key={q.id} href={`/admin/quotes/${q.id}`} className="flex items-start justify-between p-4 hover:bg-slate-50 transition-colors">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="font-mono text-xs text-cobalt-600">{q.quote_number}</span>
                    <span className={`inline-flex px-1.5 py-0.5 text-xs font-medium rounded-full ${STATUS_COLORS[q.status]}`}>
                      {STATUS_LABELS[q.status]}
                    </span>
                  </div>
                  <p className="font-semibold text-navy-900 text-sm truncate">{q.company_name}</p>
                  <p className="text-xs text-slate-500 mt-0.5">{q.contact_name}</p>
                </div>
                <div className="text-right ml-3 shrink-0">
                  <p className="font-bold text-sm text-navy-900">{fmt(q.total)}</p>
                  <p className="text-xs text-slate-400 mt-0.5">{new Date(q.created_at).toLocaleDateString("ko-KR", { timeZone: "Asia/Seoul" })}</p>
                </div>
              </Link>
            ))}
          </div>
