...
export default async function CustomerDetailPage({ params }: PageProps) {
  ...
  return (
    <div>
      <Link href="/admin/customers" className="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-primary mb-6 transition-colors">
      ...
      <div className="flex items-start justify-between gap-3 flex-wrap mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            {c.business_number && (
              <p className="text-sm text-gray-400 font-mono">{c.business_number}</p>
            )}
          </div>
        </div>
        <div className="flex gap-2">
          <Link
            ...
            className="inline-flex items-center gap-1.5 px-3 py-2 text-sm font-medium bg-primary text-white rounded-lg hover:bg-secondary transition-colors"
          >
          ...
          <Link
            ...
            className="inline-flex items-center gap-1.5 px-3 py-2 text-sm border border-gray-200 rounded-lg text-gray-700 hover:bg-gray-50 transition-colors"
          >
            수정
          </Link>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
            ...
            <div className="space-y-3">
              ...
              {c.contact_phone && (
                <div className="flex gap-3 py-2 border-b border-gray-50 last:border-0">
                  ...
                  <div>
                    <p className="text-xs text-gray-400 font-medium uppercase tracking-wide mb-0.5">연락처</p>
                    <a href={`tel:${c.contact_phone}`} className="text-sm text-primary font-medium hover:underline">{c.contact_phone}</a>
                  </div>
                </div>
              )}
              {c.contact_email && (
                <div className="flex gap-3 py-2 border-b border-gray-50 last:border-0">
                  ...
                  <div>
                    <p className="text-xs text-gray-400 font-medium uppercase tracking-wide mb-0.5">이메일</p>
                    <a href={`mailto:${c.contact_email}`} className="text-sm text-primary hover:underline truncate block">{c.contact_email}</a>
                  </div>
                </div>
              )}
            </div>
          ...
          {c.waste_types.length > 0 && (
            <div className="bg-white rounded-xl shadow-sm p-5">
              <h2 className="text-sm font-bold text-gray-700 uppercase tracking-wide mb-3">주요 폐기물</h2>
              <div className="flex flex-wrap gap-1.5">
                {c.waste_types.map((wt) => (
                  <span key={wt} className="px-2.5 py-1 text-xs bg-primary/8 text-primary rounded-full font-medium">{wt}</span>
                ))}
              </div>
            </div>
          )}
          ...
          {c.notes && (
            <div className="bg-white rounded-xl shadow-sm p-5">
              <h2 className="text-sm font-bold text-gray-700 uppercase tracking-wide mb-2">메모</h2>
        ...
        <div className="lg:col-span-2">
          <div className="bg-white rounded-xl shadow-sm overflow-hidden">
            <div className="flex items-center justify-between p-5 border-b border-gray-50">
              <h2 className="text-sm font-bold text-gray-700 uppercase tracking-wide">수거 이력</h2>
              <Link href={`/admin/dispatches?customer_id=${c.id}`} className="text-xs text-primary hover:underline">전체 보기</Link>
            </div>
            ...
            {dispatches.length === 0 ? (
              ...
              <div className="divide-y divide-gray-50">
                {dispatches.slice(0, 10).map((d) => (
                  <Link key={d.id} href={`/admin/dispatches/${d.id}`} className="flex items-center justify-between p-4 hover:bg-gray-50 transition-colors group">
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 mb-0.5">
                        <span className="font-mono text-xs text-primary">{d.dispatch_number}</span>
                    ...
                    <div className="text-right ml-3 shrink-0">
                      <p className="text-sm font-semibold text-gray-900">{fmt(d.total)}</p>
                      <ChevronRight className="w-4 h-4 text-gray-300 ml-auto mt-1 group-hover:text-primary transition-colors" />
                    </div>
                  </Link>
                ))}
              </div>
