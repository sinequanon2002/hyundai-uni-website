  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-bold text-gray-900">세금계산서 관리</h1>
          <p className="text-sm text-gray-500 mt-0.5">수거 완료 시 자동 생성 · 총 {total}건</p>
        </div>
      </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50">
                  <th className="text-left px-4 py-3 text-xs text-gray-500 font-medium">계산서 번호</th>
                  <th className="text-left px-4 py-3 text-xs text-gray-500 font-medium">수거 번호</th>
                  ...
                  <th className="text-left px-4 py-3 text-xs text-gray-500 font-medium">발행일</th>
                  <th className="text-left px-4 py-3 text-xs text-gray-500 font-medium">납부기한</th>
                  <th className="px-4 py-3 text-xs text-gray-500 font-medium">처리</th>
                </tr>
              </thead>
                ...
                {invoices.map((inv) => (
                  <tr key={inv.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-4 py-3">
                      <span className="font-mono text-xs text-primary font-medium">{inv.invoice_number}</span>
                    </td>
                    <td className="px-4 py-3">
                      {inv.dispatch_number
                        ? <Link href={`/admin/dispatches/${(inv as Record<string,unknown>).dispatch_id as string ?? ""}`} className="font-mono text-xs text-gray-500 hover:text-primary">{inv.dispatch_number}</Link>
              ...
              <div key={inv.id} className="p-4">
                <div className="flex items-start justify-between mb-2">
                  <div>
                    ...
                    <p className="font-semibold text-gray-900 text-sm mt-0.5">{inv.customer_name ?? "-"}</p>
                  </div>
                  <span className={`inline-flex px-2 py-0.5 text-xs font-medium rounded-full ${STATUS_COLORS[inv.status]}`}>
                    {STATUS_LABELS[inv.status]}
                  </span>
