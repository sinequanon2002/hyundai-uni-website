...
export default async function DispatchDetailPage({ params }: PageProps) {
  ...
  return (
    <div>
      <Link href="/admin/dispatches" className="inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-cobalt-600 mb-6 transition-colors">
      ...
      <div className="flex items-start justify-between gap-3 flex-wrap mb-6">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span className="font-mono text-sm font-bold text-cobalt-600">{d.dispatch_number}</span>
        ...
        <div className="flex items-center gap-2">
          ...
          <Link
            ...
            className="px-3 py-2 text-sm border border-slate-200 rounded-lg text-slate-700 hover:bg-slate-50 transition-colors"
          >
            수정
          </Link>
        </div>
            ...
            <div className="space-y-3">
              {d.customer_name && (
                <InfoRow icon={Truck} label="배출 사업장">
                  {d.customer_id
                    ? <Link href={`/admin/customers/${d.customer_id}`} className="text-cobalt-600 hover:underline font-medium">{d.customer_name}</Link>
              ...
              {d.created_by_name && (
                <InfoRow icon={User} label="등록자">
                  <span className="text-slate-500">{d.created_by_name}</span>
                </InfoRow>
              )}
            </div>
          </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="bg-white rounded-xl shadow-ds-sm overflow-hidden">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-slate-50">
                  <tr>
                    <th className="text-left px-4 py-2.5 text-xs text-slate-500 font-medium">폐기물 종류</th>
                    <th className="text-center px-4 py-2.5 text-xs text-slate-500 font-medium">단위</th>
                    <th className="text-right px-4 py-2.5 text-xs text-slate-500 font-medium">예상 수량</th>
                ...
                <tbody className="divide-y divide-slate-50">
                  {d.waste_items.map((item, i) => (
                    <tr key={i} className="hover:bg-slate-50">
                      <td className="px-4 py-3 font-medium text-navy-900">{item.waste_type}</td>
                      <td className="px-4 py-3 text-center text-slate-500">{item.unit}</td>
                      <td className="px-4 py-3 text-right text-slate-700">{item.estimated_qty}</td>
                      <td className="px-4 py-3 text-right text-slate-700">{fmt(item.unit_price)}</td>
                      <td className="px-4 py-3 text-right font-semibold text-navy-900">{fmt(item.amount)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="p-5 border-t border-slate-100 space-y-2">
              ...
              <div className="flex justify-between text-base font-bold text-navy-900 pt-2 border-t border-slate-100">
                <span>합계</span>
                <span className="text-cobalt-600">{fmt(d.total)}</span>
              </div>
            </div>
          </div>
        </div>
