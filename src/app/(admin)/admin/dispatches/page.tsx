  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-bold text-gray-900">수거 건 관리</h1>
      ...
      {dispatches.length === 0 ? (
        ...
        <div className="bg-white rounded-xl shadow-sm overflow-hidden">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50">
                  <th className="text-left px-4 py-3 text-xs text-gray-500 font-medium">수거번호</th>
                  ...
                  <th className="text-left px-4 py-3 text-xs text-gray-500 font-medium">예정일</th>
                  <th className="text-left px-4 py-3 text-xs text-gray-500 font-medium">담당자</th>
                  <th className="text-left px-4 py-3 text-xs text-gray-500 font-medium">폐기물</th>
                ...
                {dispatches.map((d) => (
                  <tr key={d.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-4 py-3">
                      <Link href={`/admin/dispatches/${d.id}`} className="font-mono text-xs text-primary hover:text-secondary font-medium">
                        {d.dispatch_number}
                      </Link>
                    </td>
                    <td className="px-4 py-3">
                      <Link href={`/admin/dispatches/${d.id}`} className="font-medium text-gray-900 hover:text-primary">
                        {d.customer_name ?? <span className="text-gray-400 italic">미지정</span>}
                      </Link>
                    </td>
          ...
          <div className="sm:hidden divide-y divide-gray-100">
            {dispatches.map((d) => (
              <Link key={d.id} href={`/admin/dispatches/${d.id}`} className="flex items-start justify-between p-4 hover:bg-gray-50 transition-colors">
                ...
                <div className="text-right ml-3 shrink-0">
                  <p className="font-bold text-sm text-gray-900">{fmt(d.total)}</p>
                </div>
              </Link>
            ))}
          </div>
