  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-navy-900">소개서 신청 관리</h1>
                ...
                <li key={req.id} className="bg-white rounded-xl shadow-ds-sm p-4">
                  <div className="flex items-start justify-between gap-3 mb-2.5">
                    <div className="min-w-0">
                      ...
                      <p className="text-[13px] text-slate-500 mt-0.5 truncate">{req.company_name}</p>
                    </div>
                  ...
                  <dl className="grid grid-cols-[auto_1fr] gap-x-3 gap-y-1 text-[13px]">
                    ...
                    <dd className="text-slate-700 break-all">{req.email}</dd>
                    <dt className="text-slate-400">연락처</dt>
                    <dd className="text-slate-700 tabular-nums">{req.phone}</dd>
                    <dt className="text-slate-400">마케팅</dt>
                    <dd className="text-slate-700">{req.marketing_consent ? "동의" : "미동의"}</dd>
                  </dl>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-100">
                    ...
                    <th className="text-left px-4 py-3 font-semibold text-slate-600 hidden md:table-cell">이메일</th>
                  ...
                  {requests.map((req) => {
                    ...
                    return (
                      <tr key={req.id} className="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td className="px-4 py-3 text-slate-600 hidden md:table-cell">{req.email}</td>
                        <td className="px-4 py-3 text-slate-600 hidden lg:table-cell">{req.phone}</td>
