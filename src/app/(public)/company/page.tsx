...
function SectionHeader({ tag, title }: { tag: string; title: string }) {
  return (
    <div className="text-center mb-12">
      <span className="text-cobalt-600 font-bold tracking-wider text-sm mb-2 block">{tag}</span>
  ...
  return (
    <main className="min-h-dvh bg-white">
          ...
          <nav className="flex items-center overflow-x-auto no-scrollbar">
            {ANCHORS.map((item) => (
              <a
                key={item.href}
                href={item.href}
                className="shrink-0 px-5 py-4 text-sm font-medium text-slate-500 hover:text-cobalt-600 transition-colors border-b-2 border-transparent hover:border-cobalt-600"
              >
                {item.label}
              </a>
            ))}
      ...
      <section id="greeting" className="max-w-3xl mx-auto px-4 py-16 md:py-24">
        <div className="mb-10">
          <span className="text-cobalt-600 font-bold tracking-wider text-sm mb-2 block">GREETING</span>
          <h2 className="text-3xl md:text-4xl font-bold text-navy-900 leading-tight">
            고객과 환경을 먼저 생각합니다
          </h2>
          <div className="w-12 h-1 bg-mint-500 mt-6" />
        </div>
        <div className="space-y-7 text-navy-700 leading-[1.9] text-[1.05rem] break-keep">
        ...
        <div className="mt-14 pt-8 border-t border-slate-200 text-right">
      ...
      <section id="overview" className="max-w-6xl mx-auto px-4 py-16 md:py-24 space-y-20">
            ...
            {VISIONS.map((vision, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <h3 className="text-xl font-bold text-navy-900 mb-4">{vision.title}</h3>
                  <p className="text-slate-500 leading-relaxed">{vision.description}</p>
                </div>
              );
            })}
          </div>
        ...
        {/* 기업 개요 */}
        <div>
          <h2 className="text-2xl font-bold text-navy-900 border-b-2 border-cobalt-600 pb-4 inline-block mb-8">
            기업 개요
          </h2>
          <div className="bg-white border-t-2 border-cobalt-600 shadow-ds-sm rounded-sm">
            <dl className="grid grid-cols-1 md:grid-cols-2">
              {[
                ...
                <div key={dt} className="flex border-b border-slate-200 p-4 hover:bg-slate-50 transition-colors">
                  <dt className="w-36 flex-shrink-0 font-bold text-navy-900">{dt}</dt>
                  <dd className="text-slate-500">{dd}</dd>
                </div>
              ))}
              <div className="flex border-b border-slate-200 p-4 md:col-span-2 hover:bg-slate-50 transition-colors">
                <dt className="w-36 flex-shrink-0 font-bold text-navy-900">소재지</dt>
                <dd className="text-slate-500">{COMPANY.address}</dd>
              </div>
              <div className="flex border-b border-slate-200 p-4 md:col-span-2 hover:bg-slate-50 transition-colors">
                <dt className="w-36 flex-shrink-0 font-bold text-navy-900">연락처</dt>
        ...
        <div>
          ...
          <div className="overflow-x-auto shadow-ds-sm rounded-sm">
            <table className="w-full text-left border-collapse min-w-[480px]">
              <thead>
                <tr className="bg-slate-100 border-t-2 border-cobalt-600 border-b border-slate-200">
                  <th className="py-4 px-6 font-bold text-navy-900">차량번호</th>
                  <th className="py-4 px-6 font-bold text-navy-900">허가번호</th>
                  <th className="py-4 px-6 font-bold text-navy-900">적재중량</th>
                  <th className="py-4 px-6 font-bold text-navy-900">용도</th>
                </tr>
              </thead>
              <tbody>
                {VEHICLES.map((v) => (
                  <tr key={v.plate} className="border-b border-slate-200 hover:bg-slate-50 transition-colors bg-white">
                    <td className="py-4 px-6 font-medium text-navy-900">{v.plate}</td>
                    <td className="py-4 px-6 text-slate-500">{v.licenseNo}</td>
                    <td className="py-4 px-6 text-slate-500">{v.tons}</td>
                    <td className="py-4 px-6 text-slate-500">액상 폐기물 수집·운반</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <section id="history" className="max-w-4xl mx-auto px-4 py-16 md:py-24">
        ...
        <div className="relative">
          <div className="absolute left-4 md:left-1/2 top-0 bottom-0 w-0.5 bg-cobalt-600/20 transform md:-translate-x-1/2" />
          <div className="space-y-12 md:space-y-6">
            {HISTORY_DATA.map((item, idx) => {
              ...
              return (
                ...
                >
                  <div className="absolute left-4 md:left-1/2 w-4 h-4 rounded-full bg-cobalt-600 transform -translate-x-[7px] md:-translate-x-1/2 mt-2 md:mt-0 ring-4 ring-white z-10 shadow-ds-sm" />
                  <div className={`ml-12 md:ml-0 w-[calc(100%-3rem)] md:w-1/2 pt-0.5 md:pt-4 pb-4 ${isEven ? 'md:pl-16' : 'md:pr-16 md:text-right'}`}>
                    <h3 className="text-4xl font-extrabold text-cobalt-600 mb-4 tracking-tighter opacity-90">
                      {item.year}
                    </h3>
                    <ul className="space-y-3 text-lg text-navy-700">
                      {item.events.map((event, i) => (
                        <li
                          ...
                          className={`relative break-keep leading-relaxed
                            ...
                            before:absolute before:top-2.5 before:w-1.5 before:h-1.5 before:bg-mint-500 before:rounded-full`}
                        >
                          {event}
                        </li>
                      ))}
      ...
      <section id="location" className="max-w-6xl mx-auto px-4 py-16 md:py-24">
        ...
        <div className="w-full h-[400px] md:h-[480px] rounded-xl overflow-hidden shadow-ds-md mb-16 relative bg-slate-100 flex flex-col items-center justify-center gap-6">
          ...
          <p className="text-sm text-slate-400">지도를 클릭하면 네이버지도에서 길찾기를 이용하실 수 있습니다.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div className="bg-slate-50 p-8 rounded-xl border border-slate-200 flex flex-col h-full">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h3 className="text-xl font-bold text-navy-900">주소 / 연락처</h3>
            </div>
            <div className="space-y-4 text-slate-500 leading-relaxed">
              <div>
                <strong className="text-navy-900 block mb-1">주소</strong>
                {COMPANY.address}
              </div>
              <div>
                <strong className="text-navy-900 block mb-1">연락처</strong>
                TEL: {COMPANY.tel}<br />
                Mobile: {COMPANY.mobile}<br />
                E-mail: {COMPANY.email}
              </div>
              <div>
                <strong className="text-navy-900 block mb-1">운영시간</strong>
          ...
          <div className="bg-slate-50 p-8 rounded-xl border border-slate-200 flex flex-col h-full">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h3 className="text-xl font-bold text-navy-900">대중교통</h3>
            </div>
            <div className="space-y-4 text-slate-500 leading-relaxed">
              <div>
                <strong className="text-navy-900 block mb-1">버스</strong>
                <p>경산시 하양읍 하양로 34 인근 정류장 하차 후 도보 이동</p>
              </div>
              <div>
                <strong className="text-navy-900 block mb-1">기차</strong>
          ...
          <div className="bg-slate-50 p-8 rounded-xl border border-slate-200 flex flex-col h-full">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h3 className="text-xl font-bold text-navy-900">자가용 / 주차</h3>
            </div>
            <div className="space-y-4 text-slate-500 leading-relaxed">
              <div>
                <strong className="text-navy-900 block mb-1">네비게이션</strong>
                <p>&apos;{COMPANY.shortName}&apos; 또는 &apos;하양로 34&apos; 검색</p>
              </div>
              <div>
                <strong className="text-navy-900 block mb-1">고속도로</strong>
